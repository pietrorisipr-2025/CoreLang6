...
def extract_file(container_path: str, target_path: str, out_file: str, byte_range: Optional[str] = None) -> str:
    ...
    with cp.open("rb") as fp:
        ...
        # build map cid -> (orig_len, payload_start, comp_len, codec, frame_start)
        ...
        with outp.open("wb") as w:
            for s in segs:
                ...
                if seg_end <= start or seg_start >= end:
                    continue
                # read & decode this chunk frame properly
                fp.seek(ch["frame_start"])
...
def extract_file_fast(container_path: str, toc_path: str, target_path: str, out_file: str, byte_range: Optional[str] = None, verify_chunks: bool = False) -> str:
    """
    Estrai velocemente un file usando TOC (versione 1 o 2). Se `verify_chunks=True` e TOC v2,
    calcola sha256 del chunk decompresso e confronta con il TOC.
    """
    ...
    cp = Path(container_path)
    T = _j.loads(Path(toc_path).read_text())
    entry = pick_file_entry(T, target_path)
    is_v2 = T.get("version") == 2
    ...
    with cp.open("rb") as fp, outp.open("wb") as w:
        for i, cid in enumerate(cids):
            ch = T["chunks"][cid]
            olen = int(ch["orig_len"])
            seg_start = int(offsets[i])
            seg_end = seg_start + olen
            if seg_end <= start or seg_start >= end:
                continue

            fp.seek(int(ch.get("frame_start") or ch["payload_start"]))
            fr = next(read_frames(fp))
            b = fr.payload
            name_len = b[32]
