...
from cl6b.toc import build_toc, build_toc_v2
...
def main(argv=None):
    ...
    sxf.add_argument("--container", required=True); sxf.add_argument("--path", required=True); sxf.add_argument("--out-file", required=True); sxf.add_argument("--toc", default=None); sxf.add_argument("--verify", action="store_true")
    ...
    stoc = sub.add_parser("build-toc")
        ...
        stoc2 = sub.add_parser("build-toc-v2"); stoc2.add_argument("--container", required=True); stoc2.add_argument("--out", default=None); stoc2.set_defaults(func=cmd_build_toc_v2)
...
vf = sub.add_parser("verify-file"); vf.add_argument("--container", required=True); vf.add_argument("--toc", required=True); vf.add_argument("--path", required=True); vf.add_argument("--out"); vf.set_defaults(func=cmd_verify_file)
...
def cmd_build_toc_v2(a):
    out = build_toc_v2(a.container, a.out)
    print(out); return 0


def cmd_verify_file(a):
    import json, hashlib, pathlib
    T = json.loads(pathlib.Path(a.toc).read_text())
    ...
    # Ricostruisci e hasha via extract (range intero)
    tmp = pathlib.Path(a.out or ".cl6_verify.tmp")
    from cl6b.partial import extract_file_fast
    extract_file_fast(a.container, a.toc, a.path, str(tmp), None, verify_chunks=True)
    got = hashlib.sha256(tmp.read_bytes()).hexdigest()
    ok = (got == entry["sha256"])
    rep = {"path": a.path, "expected": entry["sha256"], "got": got, "ok": ok}
    tmp.unlink(missing_ok=True)
    import json as _j; print(_j.dumps(rep, indent=2)); return 0 if ok else 2
